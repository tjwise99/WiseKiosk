...
- `just check-reqs`  — the Doorstop requirements tree validates: no item carries a review
  fingerprint nobody wrote, refs resolve, no suspect/unreviewed/orphan items; and no inactive item
  has drifted from the stamp it was reviewed at, its parent UIDs included — Doorstop reports neither
  stamp signal for an inactive item, so editing a pending one fails the gate until it is re-stamped;

...
- `just check-reqs`  — the Doorstop requirements tree validates: no item carries a review
  fingerprint nobody wrote, a hand-declared `true` included; refs resolve; no suspect/unreviewed/orphan
  items, with suspect links checked on the inactive items Doorstop skips — orphan detection is not, so
  a pending item with no parent is reported by nothing; one exception is tolerated while every `TST` item is still

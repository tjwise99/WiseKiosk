...
visibly apart (SRS045, present and outlook shown apart; hours apart from days). Each group is drawn
with a **different treatment**, which is what carries that separation — a block, a curve, a strip —
so the distinction survives even where a grouping stroke would not.
...
This carries the whole present set SRS044 puts across the boundary (condition, day-or-night,
temperature, apparent temperature, humidity, wind). Day or night is carried by *which glyph is drawn*
(SRS050, day and night apart; the styling contract's *Typeface*), never by dimming or colour.
...
- directly beneath, a **strip** of five cells aligned to the vertices, each carrying the hour, its
  condition glyph (day or night, per SRS050), and its precipitation chance.
...
- **Unavailable** — the failure's own plain-language message at the `body` step, in the module's own
  place, affecting no other module (SRS001, a failed module shows why, and only that module).
...
|---|---|
| present / hours / days each a different treatment | SRS045 — present and outlook apart; hours apart from days |
| present glyph, hourly glyph, per day | SRS050 — day and night apart (via the glyph) |
| present block carries condition, temp, feels, humidity, wind | SRS044 — the present set across the boundary |
| full-white curve, glyphs and figures | SRS032 / SRS030 — content at full emission, nothing else above the ceiling |
| every step at or above the scale's floor | SRS033 — text holds a minimum size against the display |

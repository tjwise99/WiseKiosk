...
| [`../scripts/README.md`](../scripts/README.md) | The inputs each repository check has been exercised against, in both directions — the defect it must catch and the legal input it must not reject — and how to re-run one | What a check asserts, and why (`CI.md`); which tier a test belongs to (`TESTING.md`) |

...
| [`CI.md`](CI.md) | The gate regime — which checks block a merge, what each is allowed to let through, and how a finding nobody can fix is disposed of, with the reasoning for each stance | The test architecture (`TESTING.md`); the checks themselves (the requirements tree, as `TST` items); how a change gets reviewed by a person (`../CONTRIBUTING.md`) |

...
| [`contracts/`](contracts/module-contract.md) | The prose form of a contract an author follows by hand, where the requirements state the obligation but not the shape — currently the six-part module contract | The obligation itself (the requirements tree); the per-module test obligations (`TESTING.md`) |

...
| [`DEPLOYMENT.md`](DEPLOYMENT.md) | **What a deployment must do** to reach a running kiosk and to move between published digests: what an operator receives, the obligations on the documented bring-up procedure, the defaults the shipped recipe carries, and the position on configuration compatibility across an upgrade. The canonical home for an obligation on what ships beside the image rather than on the kiosk itself | An obligation on the running software (the requirements tree); the check that decides any of them, and what a release publishes (`CI.md`); a decision with a rejected alternative (an ADR) |

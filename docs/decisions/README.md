...
| [0008](0008-boundary-contract-openapi-codegen.md) | 5 | 2026-08-31 | Boundary contract: one OpenAPI schema (3.0.3 now, 3.1 later), the whole wire contract — routes, client, server and types — generated from it on both sides, CI drift-gated and compiled |
...
| [0010](0010-runtime-materialised-gate-fixtures.md) | 2 | 2026-09-05 | No vulnerable artifact is ever committed in resolvable form; a negative fixture is built in a throwaway copy and its finding recorded once, rather than re-tested by a standing meta-gate |
...
| [0021](0021-repository-layout.md) | 3 | 2026-08-30 | The top level projects the containers: a Go module root, an npm package root, the boundary schema owned by neither, and the release material outside both; a module's files split across the packages that run them, under one name |
| [0022](0022-config-schema-format.md) | 2 | 2026-08-30 | Configuration schema authored as JSON Schema 2020-12: one hand-authored document carrying a named section per module, enforced by a single in-page validator, TypeScript types generated from it |
...
| [0029](0029-api-explorer-swagger-ui.md) | 1 | 2026-09-09 | The API explorer renders `boundary/openapi.yaml` with a build-fetched, never-committed Swagger UI; interactive locally via a self-contained dev server in `docs/site` (`just docs-serve`, :5174, no backend or frontend change), browsable-only on the published site |

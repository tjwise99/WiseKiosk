...
**Status:** accepted
**Decided:** 2026-09-15 (rev 6's generated-code-is-never-committed principle taken on #339
generated-code-not-committed; rev 4's input shape and seam location, both taken at the first module
data route on #220 weather module; the types-to-wire-contract pivot taken 2026-08-23 in the #188
boundary codegen session, and the surrounding model 2026-07-23 at the boundary-contract requirements
round #37, with the codegen-mechanism trade carried by #7 boundary-contract codegen). This ADR
records the mechanism **decision**; the **build** is #7 boundary-contract codegen, and rev 3's
migration is #188 boundary codegen.
**Rev:** 6

## Revisions

- **rev 6** — 2026-09-15 — authors the general principle that no autogenerated file is committed
  except rendered diagrams: `boundary.gen.go` and `client.ts` are generated at build, when missing,
  under each side's own toolchain, and gitignored rather than committed. Drops the drift-gate
  mechanism and every reference to it; the real rejected alternative — commit the output and gate it
  with a diff check, the status quo through rev 5 — is recorded with PR #321's drift cost as the
  reason it was rejected. This changes what is decided, so the `Decided` date moves with it (#339
...
which makes the boundary contract non-negotiable: **one schema, both sides generated from it at
build, never committed** — a malformed schema or a generator that cannot run fails the build itself,
rather than a drift gate catching a committed copy after the fact. Round #37 elicited the requirements for that property
...
- **Each side generates its whole wire contract — routes, client, server and types — not types
  ...
  single-definition rule is about *outside* what either generator emits at all. A generated *type*
  naming a path is not enough: the request is issued and served by hand-written strings, so nothing
  generated ever disagrees with itself while the two sides silently disagree with each other. Rev 2
  had it that way, and the first liveness route promptly grew a bespoke per-route drift script to
  patch the hole by hand. Generating the route table closes it by construction — the registration and
  the call site are both output of the schema, so a path that moves in the schema moves identically on
  both sides, with no hand-authored copy able to lag behind.
...
- **Generated at build, when missing, never committed.** Each side's own recipe checks whether its
  generated file is present and, if not, invokes that side's generator under that side's own
  toolchain — Go under `go tool oapi-codegen`, TypeScript under `orval`. Neither generated artifact is
  committed, so there is no copy left behind for a generator bump to drift from: a Renovate pull
  request bumping the pinned generator version needed a separate, manual regenerate-and-commit before
  this shape (PR #321), a step this shape has no committed target for at all. The generators stay
  **version-pinned** regardless, for reproducibility rather than to guard a diff. **Both languages are
  compiled, not merely written**: a generator that exits zero on a configuration it merely accepts, or
  that emits an empty or partial file, is caught by the consumer —
  `go build` and the whole-project `tsc --noEmit` — because a missing declaration is an undefined
  symbol wherever it is consumed (SRS016<!-- Both sides consume the generated types -->). A malformed
  schema or an unresolvable generator therefore fails the **build** outright, never a standing drift
  gate.
...
- **Commit the generated output and gate it with a diff check** — the status quo through rev 5:
  regenerate both sides in CI, fail on any difference from the committed copy. Rejected: a committed
  artifact is coupled to the exact generator version that produced it, and nothing keeps that coupling
  honest. PR #321, a Renovate pull request bumping orval's pinned version, bumped the version string
  alone — the generator was never re-run, so the committed client silently fell out of step with what
  the new version would emit, needing a separate, manual regenerate-and-commit to close. Generating at
  build, with nothing committed, removes the coupling rather than policing it: there is no committed
  copy for a version bump to leave behind.
...
- **TypeSpec → OpenAPI** — a more ergonomic authoring DSL, but it adds a Node compile hop and a
  third build-time generation stage feeding the two already there, and makes the OpenAPI itself a
  generated artifact. An authoring abstraction whose only consumer is ~5 routes — generality ahead of a second use
...
- **User templates overriding the binding** — rejected. It would mean vendoring copies of two
  upstream templates pinned to this generator's version, and every build regenerates *through* them,
  so an upstream template change would take effect silently, invisible to anything but the generator's
  own version pin.
...
- **Two pinned generators to keep current** — inherent to Go not sharing types (ADR 0001 rev 1 paid for
  ...
  rather than a reformat every time the tool moves.
- **A route cannot be added by hand on either side.** Adding one means editing the schema and
  regenerating; the generated files are gitignored, so a hand-added registration or call cannot reach
  a commit at all, and a local edit is silently discarded the next time the file is deleted and
  regenerated. That is the point, and it is also the cost — the schema sits on the critical path of
  every route change, not only of every payload change.

...
[ADR 0006 rev 2](0006-process-gates.md) specifies exactly one property of a ticket — that it is open and
...
branch is a branch, so under ADR 0006 rev 2 it links an open type-labelled issue of its own, and that issue
...
- **Detect-and-flag on `issues:` events** — an Action labelling `orphan`/`needs-triage`, which is
  what #64 work-tracking invariants proposed. Rejected: it needs `issues: write`, and ADR 0006 rev 2
...
- **A ticket nobody works is never seen.** The gate reaches an issue only when a branch names it, so
  ...
  progress. This is the cost of holding ADR 0006 rev 2's read-only line, and it is accepted knowingly.
...
- **The write-time half is advisory by construction.** Nothing at filing time refuses a malformed
  ...
  the half with teeth, and the pairing is ADR 0006 rev 2's own — an advisory `commit-msg` hook beside a

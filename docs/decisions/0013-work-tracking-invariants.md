...
branch is a branch, so under ADR 0006 it links an open type-labelled issue of its own, and that issue
...
[ADR 0011](0011-requirement-or-convention.md)'s mistake, one level up from where it usually happens.
...
- **Detect-and-flag on `issues:` events** — an Action labelling `orphan`/`needs-triage`, which is
  what #64 work-tracking invariants proposed. Rejected: it needs `issues: write`, and ADR 0006
...
- **Writing these as requirements in the tree.** Rejected under ADR 0011: nothing the running kiosk
...
- **A ticket nobody works is never seen.** The gate reaches an issue only when a branch names it, so
  ...
  progress. This is the cost of holding ADR 0006's read-only line, and it is accepted knowingly.

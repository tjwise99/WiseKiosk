...
in-source directives — see [ADR 0004 rev 3](../decisions/0004-docs-site-sphinx-needs.md)), the repo-root
...
(ADR 0004 rev 3): each is a pure `{include}` of its repo-root counterpart, using MyST's `relative-docs`
...
reaches them via a glob toctree — structure, not presentation, so the ADR 0004 rev 3 thin-transform
boundary holds.
...
`checks.yml` stays read-only (ADR 0004 rev 3).

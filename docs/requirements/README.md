...
| Stamp a new item | `doorstop review <UID>`, then `doorstop clear <UID>` | Two stamps, and `review` writes only one of them. It stamps the item body and leaves `links:` reading `- SYS0MM: null`; `clear` is what writes the parent fingerprint into the link. `check-unreviewed.py` fails on the null, so an item added, linked and reviewed but never cleared reds the gate — the order is the reverse of the re-blessing row below, where the link stamp is refreshed before the item is re-read |

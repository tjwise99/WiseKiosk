...
**The four method names are the whole permitted set, and a value outside it is a defect rather than
an exemption.** An unrecognised spelling — a typo, a capitalised token, or the attribute missing —
ranks nowhere in the decidability order, so a rule that merely skipped what it could not rank would
excuse the item from the comparison and report the tree consistent.

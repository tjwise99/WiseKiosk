...
| **Render** | Each module renders from its props; the page assembles with a known-good config; and the assembled page carries the values a viewer depends on: content too large for its region overflows rather than being hidden, text sits at full emission and above the type-scale floor, laid-out regions clear the display edge, and unoccupied surface stays at minimum emission | [module contract](contracts/module-contract.md), part 3 / SRS017<!-- Full-screen assembly at kiosk; reflow, no horizontal scroll, at narrower widths --> / SRS030<!-- Unoccupied surface is held at minimum emission --> / SRS032<!-- Readable content is carried at full emission --> / SRS033<!-- Text holds a minimum size against the display, at every resolution --> / SRS034<!-- The laid-out regions keep clear of the display edge --> | Every commit, in CI |
...
already **SRS001<!-- A failed module shows why, and only that module -->**, and
`TST001`<!-- Pending: upstream-failure error payload test --> names malformed payload as one of its

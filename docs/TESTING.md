...
- **A verification item's fit against its parent is re-read when the item is activated.** Every `TST`
  ...
  mirror case is the check itself moving to a different parent, which no fingerprint can see because
  the stamp does not cover `links`; `check-reparent-review.py` fails a parent set that moved without a
  fresh review. What neither can decide is the question activation exists to ask, and that stays a

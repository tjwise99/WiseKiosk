...
- **API keys resolve server-side only** and never reach the browser by construction — no code path
  carries a secret toward a client, and confinement is never a stripping step (SYS017).
- **Secrets are delivered by the deployment environment**, never stored in the image, the
  repository, or the configuration (SYS018, SRS015).
...
- **CI holds no credentials**; verification needing a live key runs outside CI (SYS019, SRS019).
- **The browser enforces the page's posture**: a same-origin content-security policy and the
  standard hardening headers (SYS030, SRS065, SRS066).

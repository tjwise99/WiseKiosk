...
func main() {
	...
	if *selfCheck {
		if err := health.Check("http://localhost" + addr + healthPath); err != nil {
...
// everything else. A nil api answers 404 for every path under /api/.
func newServer(static, api http.Handler) http.Handler {
	...
	mux.Handle(healthPath, health.Handler())
	mux.Handle("/api/", api)
	mux.Handle("/", static)
	return mux
}

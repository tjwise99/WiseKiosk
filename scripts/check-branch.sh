...
# and exactly one type label, which is the branch type (ADR 0013 rev 2). main and
...
if [ "$base_ref" = "$default_branch" ]; then
    [ -z "$parent" ] || fail "issue #$number is a sub-issue of #$parent, but PR #$pr_number targets the default branch — sub-issue membership means a shared merge target, not topical grouping; the milestone is what groups (ADR 0013 rev 2)"
    ...
    anchor=${anchor_rest%%-*}
    [ -n "$parent" ] || fail "PR #$pr_number targets integration branch '$base_ref' but issue #$number has no parent — a ticket whose PR targets an integration branch is a sub-issue of that branch's anchor #$anchor (ADR 0013 rev 2)"

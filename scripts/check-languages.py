...
[ADR 0017 rev 7](../docs/decisions/0017-authored-language-set.md) binds two authored languages to
...
and the assets a documentation build serves, are carved out of the decision entirely — CSS excepted,
which also carries a documentation disposition beside its authored one, both named where the
extension is declared below.
...
decided about **fails**, which is the opposite of a denylist: the unbounded set ADR 0017 rev 7 rejected
would pass silently on a new extension, and only a bounded, declared set can fail on one.

`sh` and `mjs` are **not** declared extensions: ADR 0017 rev 7 names POSIX sh and Node as authoring
...
carries **a disposition rather than an exemption** — ADR 0017 rev 7's own words for this, applied here
...
EXTENSIONS = {
    # Authored — what ships (ADR 0017 rev 7 Decision table).
    "go": "authored — Go, what ships to a user or an operator (ADR 0001 rev 1, ADR 0017 rev 7)",
    "ts": "authored — TypeScript, what ships (ADR 0017 rev 7)",
    "svelte": "authored — the Svelte component format, what ships "
    "(ADR 0018 rev 1, ADR 0017 rev 7)",
    "css": "authored — the shared product stylesheet, what ships (the display styling contract, ADR 0017 rev 7); "
    "also documentation — an asset the Sphinx docs-site build serves, a Furo theme override "
    "(ADR 0017 rev 7)",
    # Authored — what checks the repository (ADR 0017 rev 7 Decision table).
    "py": "authored — Python, standard library only, what checks the repository (ADR 0017 rev 7)",
    # Derived — a toolchain's own required input format (ADR 0017 rev 7 Decision).
    "yml": "derived — a toolchain's own required input format: GitHub Actions workflow YAML, "
    "Doorstop item and silo config YAML, Dependabot YAML (ADR 0017 rev 7)",
    ...
    "read and oapi-codegen's own configuration (ADR 0008 rev 2, ADR 0017 rev 7)",
    ...
    "likec4": "derived — LikeC4's own model format, named in ADR 0017 rev 7 "
    "(ADR 0003 rev 2)",
    ...
    # Documentation, and the assets a documentation build serves — ADR 0017 rev 7 states this
    # decision does not reach them.
    "md": "documentation — not an authored program; ADR 0017 rev 7 does not reach documentation",
...
NO_EXTENSION = {
    "justfile": "derived — invoking `just`, named explicitly in ADR 0017 rev 7",
...
# NOT declared extensions: ADR 0017 rev 7 says they author nothing, so a *new* file in either must
# fail. The files below predate that decision and each carries "a disposition rather than an
# exemption" in ADR 0017 rev 7's own words — so they are grandfathered one at a time, and `main()`
...
LEGACY = {
    ...
    "retires the pending-tier exception it stands in for (#78; ADR 0017 rev 7)",
}
    ...
    for name in files:
        ...
        if extension not in EXTENSIONS:
            problems.append(
                ...
                f"authored or derived (ADR 0017 rev 7), or grandfather this path with its disposition"
            )
        else:
            judged += 1

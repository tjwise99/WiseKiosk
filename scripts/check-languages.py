...
[ADR 0017 rev 6](../docs/decisions/0017-authored-language-set.md) binds two authored languages to
...
decided about **fails**, which is the opposite of a denylist: the unbounded set ADR 0017 rev 6 rejected
would pass silently on a new extension, and only a bounded, declared set can fail on one.

`sh` and `mjs` are **not** declared extensions: ADR 0017 rev 6 names POSIX sh and Node as authoring
...
carries **a disposition rather than an exemption** — ADR 0017 rev 6's own words for this, applied here
...
EXTENSIONS = {
    # Authored — what ships (ADR 0017 rev 6 Decision table).
    "go": "authored — Go, what ships to a user or an operator (ADR 0001 rev 1, ADR 0017 rev 6)",
    ...
    "py": "authored — Python, standard library only, what checks the repository (ADR 0017 rev 6)",
    # Derived — a toolchain's own required input format (ADR 0017 rev 6 Decision).
    "yml": "derived — a toolchain's own required input format: GitHub Actions workflow YAML, "
    "Doorstop item and silo config YAML, Dependabot YAML (ADR 0017 rev 6)",
    ...
    "likec4": "derived — LikeC4's own model format, named in ADR 0017 rev 6 "
    "(ADR 0003 rev 2)",
    ...
    # Documentation, and the assets a documentation build serves — ADR 0017 rev 6 states this
    ...
    "svg": "documentation — a figure a Markdown document embeds and an asset the Sphinx docs-site "
    "build serves (ADR 0017 rev 6: 'the assets a documentation build serves')",
}
...
NO_EXTENSION = {
    "justfile": "derived — invoking `just`, named explicitly in ADR 0017 rev 6",
...
# NOT declared extensions: ADR 0017 rev 6 says they author nothing, so a *new* file in either must
# fail. The files below predate that decision and each carries "a disposition rather than an
# exemption" in ADR 0017 rev 6's own words — so they are grandfathered one at a time, and `main()`
...
LEGACY = {
    ...
    "retires the pending-tier exception it stands in for (#78; ADR 0017 rev 6)",
}
    ...
    for name in files:
        ...
        if extension not in EXTENSIONS:
            problems.append(
                ...
                f"authored or derived (ADR 0017 rev 6), or grandfather this path with its disposition"
            )
        else:
            judged += 1

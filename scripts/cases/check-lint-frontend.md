...
Each case is a `git archive` copy of the tracked tree at `51b8e5e`, with `frontend/node_modules`
...
| Must pass | The tree as it stands, a clean tree | `svelte-check --tsgo` reports zero findings against `ModuleEntry.component` and the whole tree, and `check-lint-frontend` exits 0 with both eslint and svelte-check blocking. [#275 resolve ModuleEntry.component prop-type variance so svelte-check blocks](https://github.com/tjwise99/WiseKiosk/issues/275) resolved the prop-type variance findings at `src/lib/modules.ts:33:12`, `src/lib/modules.ts:35:5` and `tests/render/stubs/registry.ts:31:18` |

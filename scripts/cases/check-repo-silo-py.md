...
resolving to the pinned `tjwise99/renovate-runner` preset.
...
`renovate.json` resolving the pinned `tjwise99/renovate-runner` preset.
...
| Must fail | extends lacks the runner preset | `extends` holds entries, none starting `github>tjwise99/renovate-runner` |
| Must fail | extends preset unpinned (no `#`) | `extends: ["github>tjwise99/renovate-runner"]`, no `#tag` |
| Must fail | file is not JSON | `renovate.json` edited to invalid JSON |
| Must pass | The real `renovate.json` | the tree as it stands, `extends: ["github>tjwise99/renovate-runner#v1.0.0"]` |

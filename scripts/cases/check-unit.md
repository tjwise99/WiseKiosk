...
generated `Region` union, so a missing or extra name is a type error — one `check-typecheck-frontend`
(docs/CI.md § *Lint and type checks*) now also catches, but these two rows still prove the roster
stays in sync at run time rather than only at compile time. Seeding both directions is what makes that
a check rather than a coincidence: a comparison of sizes would pass the second seed, and a one-way
subset test would pass the first.

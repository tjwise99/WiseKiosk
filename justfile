...
[group('checks')]
[doc('The unit-test coverage bar: go-test-coverage over the backend, Vitest coverage.thresholds over the frontend `.ts` files, and monocart over the frontend `.svelte` components from the render tier, each 90% per-file and total; needs `just boundary-install` and `just render-install`')]
check-coverage:
    ...
    frontend/node_modules/.bin/mcr merge -c frontend/mcr.config.js
